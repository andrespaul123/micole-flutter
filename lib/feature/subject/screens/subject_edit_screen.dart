...
import '../subject_viewmodel.dart';

class SubjectEditScreen extends StatefulWidget {
  final int id;

  const SubjectEditScreen({super.key, required this.id});
...
class _SubjectEditScreenState extends State<SubjectEditScreen> {
  ...
  Future<void> cargarMateria() async {
    final vm = context.read<SubjectViewModel>();

    final subject = await vm.getSubjectById(widget.id);
